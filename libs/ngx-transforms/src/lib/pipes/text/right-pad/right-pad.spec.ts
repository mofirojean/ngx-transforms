import { RightPadPipe } from './right-pad';
...
describe('RightPadPipe', () => {
  ...
  it('should pad with dots', () => {
    expect(pipe.transform('hi', 5, '.')).toBe('hi...');
  });

  it('should pad with zeros', () => {
    expect(pipe.transform('42', 5, '0')).toBe('42000');
  });
  ...
  it('should handle multi-character pad string', () => {
    expect(pipe.transform('hi', 8, 'ab')).toBe('hiababab');
  });

  it('should truncate multi-character pad correctly', () => {
    expect(pipe.transform('hi', 5, 'abc')).toBe('hiabc');
  });
