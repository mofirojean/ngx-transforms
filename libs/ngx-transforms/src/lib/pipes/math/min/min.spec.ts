...
describe('MinPipe', () => {
  ...
  it('should return the minimum number', () => {
    expect(pipe.transform([5, 3, 8, 1, 9])).toBe(1);
  });
  ...
  it('should handle negative numbers', () => {
    expect(pipe.transform([5, -3, 8, -10, 2])).toBe(-10);
  });
  ...
  it('should handle decimal numbers', () => {
    expect(pipe.transform([1.5, 0.3, 2.7, 0.1])).toBe(0.1);
  });

  it('should ignore non-number values', () => {
    expect(pipe.transform([5, 'a', 3, null, 8] as unknown[])).toBe(3);
  });
  ...
  it('should ignore NaN values', () => {
    expect(pipe.transform([5, NaN, 3, NaN])).toBe(3);
  });

  it('should find min by object key', () => {
    const products = [
      { name: 'Laptop', price: 999 },
      { name: 'Mouse', price: 29 },
      { name: 'Monitor', price: 450 },
    ];
    expect(pipe.transform(products, 'price')).toBe(29);
  });

  it('should find min by nested key', () => {
    const orders = [
      { id: 1, meta: { total: 150 } },
      { id: 2, meta: { total: 45 } },
      { id: 3, meta: { total: 200 } },
    ];
    expect(pipe.transform(orders, 'meta.total')).toBe(45);
  });
  ...
  it('should handle missing nested key gracefully', () => {
    ...
    expect(pipe.transform(input, 'a.b')).toBe(5);
  });
  ...
  it('should handle negative numbers by key', () => {
    ...
    expect(pipe.transform(temps, 'temp')).toBe(-40);
  });
